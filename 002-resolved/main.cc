...
int main(int argc, char** argv)
{
    ...
    for (auto nti: range(control.get_init_num(), nt_max+control.get_init_num()))
    {
		...
		const real_t umaxLES= spade::algs::transform_reduce(les_solution, get_u, max_op);
        real_t ub, rhob;
        ...
        if (std::isnan(umax) || std::isnan(umaxLES))
        {
            if (group.isroot())
            {
